      ...
      {loading ? <div>Loading...</div> : (
        plans.length === 0 ? <div>No plans submitted yet.</div> : plans.map((plan) => (
          <div key={plan._id} className="border rounded p-3 mb-4">
            <div className="mb-2 font-semibold">Student: {plan.student?.email || plan.student}</div>
            <ul className="mb-2">
              {plan.courses.map((c, i) => (
                <li key={i}>
                  {c.course?.name || courses.find(course => course._id === c.course)?.name || "(Unknown)"} - <span className="capitalize">{c.status}</span>
